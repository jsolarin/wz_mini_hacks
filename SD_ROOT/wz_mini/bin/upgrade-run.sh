...
setup() {
        ...
        wget --no-check-certificate https://github.com/gtxaspec/wz_mini_hacks/archive/$BRANCH.tar.gz -O /opt/.Upgrade/wz_mini.tar.gz; sync
...
case "$1" in
        ...
        ;;
        "downgrade")
                if [ ! "$2" ]; then echo "Downgrade requires the commit hash as the second argument. Exiting."; exit 1; fi
                BRANCH=$2
                setup
        ;;
        *)
